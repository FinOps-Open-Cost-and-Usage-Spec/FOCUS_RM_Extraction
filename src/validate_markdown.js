 ...
 * CLI that prints all errors and exits non-zero when any file is malformed. Point it at a
 * specification tree with --specification; the test suite passes the fixture tree the same way.
 */
...
function collectErrors(specRoot = DEFAULT_SPEC_ROOT) {
  const contract = JSON.parse(fs.readFileSync(CONTRACT_PATH, 'utf8'));
  const rel = (p) => path.relative(specRoot, p);
  const errors = [];

  const dm = contract.DataModel;
  const dmPath = specPath(specRoot, dm.Location);
  errors.push(...validateEntity(fs.readFileSync(dmPath, 'utf8'), rel(dmPath), dm.Headings));

  const dsLocation = specPath(specRoot, contract.Datasets.Location);
  for (const folder of resolveDatasetFolders(dsLocation)) {
    const ds = contract.Datasets;
    const dsPath = path.join(dsLocation, folder, 'dataset.md');
...
const USAGE = `Usage: node validate_markdown.js [options]
...
function main() {
  const opts = parseArgs(process.argv.slice(2), ['specification']);
  if (opts.help) {
    console.log(USAGE);
    return;
  }
  const specRoot = opts.specification ? path.resolve(opts.specification) : DEFAULT_SPEC_ROOT;
  if (!fs.existsSync(specRoot)) {
    throw new UsageError(`Specification not found: ${specRoot}\nPass --specification <folder>.`);
  }

  const errors = collectErrors(specRoot);
  if (errors.length === 0) {
    console.log(`✅ All in-scope markdown under ${specRoot} is structurally valid.`);
    return;
  }
